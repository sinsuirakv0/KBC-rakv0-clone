...
XAPK選択の`c`はAPKCombo、`p`はAPKPureの取得画面を開きます。どちらも画面内に表示する3段階の案内に従い、XAPKを手動でダウンロードしてください。Termuxへ戻ってEnterを押すと、今回保存したXAPKを検出して選択します。配布元のログイン、利用規約への同意、検証などは利用者自身で完了してください。
