...
セットアップ時に、日本語IMEとAndroid標準インストーラー連携に必要なTermux設定を自動で有効にします。既存の`~/.termux/termux.properties`にある他の設定は維持されます。
...
識別子には、小文字英字で始まる英小文字・数字・`_`・`.`を使用できます。アイコンは「公式」「KBCオリジナル」「PNGを指定」の3種類から選べます。指定したPNGは通常アイコンとアダプティブ前景それぞれの解像度へ自動調整され、次回更新でも自動的に再利用されます。
